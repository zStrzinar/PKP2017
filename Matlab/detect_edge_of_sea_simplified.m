function [ detector_state, sel_xy, objects, masked_sea ] = detect_edge_of_sea_simplified(detector_state, Im_in)
%#codegen
...
if initialize_detector,
    ...
    if ~isequal(detector_state.colorspace, Colorspace.hsv)
        ...
        for k = 1 : 3
            ...
            detector_state.current_mixture(k) = ...
                momentMatchPdf(detector_state.current_mixture(k).Mu,tmp_mu, detector_state.current_mixture(k).Cov, tmp_cov , w_mix) ;
        end
...
[ detector_state, Q_sum_large ] = run_SSM(detector_state, dataEM) ; 
t = toc ;
fprintf('%3.0f miliseconds required for segmentation.\n', t*1000) ;

%% Retrieve objects and sea edge
[ objects, ~, sel_xy, masked_sea ] = getEdgeAndObjectsNoScaling(Q_sum_large , Im_size);
