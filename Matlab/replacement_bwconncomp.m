function CC = replacement_bwconncomp (I, n) 
    % A MATLAB Coder friendly implementation of BWCONNCOMP
    
    % Use label implementation to label connected components
    [ L, num ] = label(I, n);
    
    CC = repmat(struct('label', 0, 'pixel_idx', [], 'area', 0), 1, num);
    coder.cstructname(CC, 'ConnectedComponent')
    coder.varsize('CC(:).pixel_idx');
    
    for i = 1:num, 
        CC(i).label = i;
        CC(i).pixel_idx = find(L(:) == i);
        CC(i).area = numel(CC(i).pixel_idx);
    end
end
...
% Label matrix
%isNaNI = isnan(I);
isNaNI = (I == 0); % We operate on logicals
id(isNaNI) = NaN;
[id,~,n] = unique(id(:));
I = 1:length(id);
L = reshape(I(n),sizI);
